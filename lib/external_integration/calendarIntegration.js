...
async function getGoogleCalendars(callback) {
    ...
    const uri = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.GOOGLE_CALENDAR_OAUTH_BEGIN, {auth: serverHandler.current.userState.auth, username: serverHandler.current.userState.username});
...
async function getOutlookCalendars(callback) {
    //fetch oauth url from the server
    const uri = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.OUTLOOK_CALENDAR_OAUTH_BEGIN,  {auth: serverHandler.current.userState.auth, username: serverHandler.current.userState.username});
    await Linking.openURL(uri);
