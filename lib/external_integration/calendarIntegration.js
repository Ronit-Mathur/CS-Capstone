...
async function importGoogleCalendar(id, username) {
    await serverHandler.current.fetchGet(SERVER_ENDPOINTS.GOOGLE_CALENDAR_IMPORT_ENDPOINT, { id: id, username: username }); //fix username integration
}
...
async function getOutlookCalendars(callback) {
    ...
    //ask the server for calendars every second. when we get a valid response, stop
    const askInt = setInterval(async () => {
        const resp = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.OUTLOOK_CALENDAR_LIST_CALENDARS);
        if (resp != "error") {
...
async function importOutlookCalendar(id, username){
    await serverHandler.current.fetchGet(SERVER_ENDPOINTS.OUTLOOK_CALENDAR_IMPORT_ENDPOINT, { id: id, username: username }); //fix username integration
