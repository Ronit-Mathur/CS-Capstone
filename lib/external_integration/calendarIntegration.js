...
async function getGoogleCalendars(callback) {
    ...
    const askInt = setInterval(async () => {
        const resp = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.GOOGLE_CALENDAR_LIST_CALENDARS, {auth: serverHandler.current.userState.auth});
        if (resp != "error") {
...
async function getOutlookCalendars(callback) {
    ...
    //ask the server for calendars every second. when we get a valid response, stop
    const askInt = setInterval(async () => {
        const resp = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.OUTLOOK_CALENDAR_LIST_CALENDARS, {auth: serverHandler.current.userState.auth});
        if (resp != "error") {
