...
async function totalRates(){
    var count = -1;
    try{
        count = await  await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_DAILY_COUNT, {username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});
