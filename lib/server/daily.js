...
async function rateDay(username, day, happiness){
    try {
        const resp = await serverHandler.current.fetchGet(serverConstants.SERVER_ENDPOINTS.USER_RATE_DAY, { username: username, day: day, happiness:happiness, auth: serverHandler.current.userState.auth });
    }
    catch (e) {
        console.log(e);
        return false;
