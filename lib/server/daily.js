...
import dayHandler from "../../backend_server/handlers/dayHandler";
...
async function rateDay(username, happiness) {
    try {
        ...
        if (await isRemoteAsync()) {
            ...
            await dayHandler.current.rateDay(serverHandler.current.userState.username, day, happiness, now);
        }
    }
    catch (e) {
        console.log(e);
...
async function getDay(day) {
    ...
    try {
        if (await isRemoteAsync()) {
            ...
            dayObj = await dayHandler.current.getDaily(serverHandler.current.userState.username, day);
        }
...
async function totalRates() {
    ...
    try {
        if (await isRemoteAsync()) {
            count = await serverHandler.current.fetchGet(serverConstants.SERVER_ENDPOINTS.USER_DAILY_COUNT, { username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth });
        } else {
            count = await dayHandler.current.totalRated(serverHandler.current.userState.username);
        }
    } catch (e) {
