...
async function completeTask(id, enjoyment, physicalActivity, engagement, mentalDifficulty) {
    var result = false;
    try {
        result = await serverHandler.current.fetchGet(serverConstants.SERVER_ENDPOINTS.USER_COMPLETE_TASK, { taskId: id, enjoyment: enjoyment, physicalActivity: physicalActivity, engagement: engagement, mentalDifficulty: mentalDifficulty, auth: serverHandler.current.userState.auth });
        return result == "ok";
    }
    catch (err) {
