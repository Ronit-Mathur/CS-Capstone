...
async function totalRatedTasks(){
    var count = -1;
    try{
        count = await  await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_TASKS_COUNT_RATED, {username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});
...
export {totalRatedTasks, totalCompletedTasks, getRatedTasksByEngagement, getUnratedCompletedTasks, addTask, getDaysTasks, getTodaysFinishedTasks, getTodaysActiveTasks, updateTask, deleteTask, rateTask, getTaskRating, getTasksByMonth, getTask, getTaskRatingsByMonth }