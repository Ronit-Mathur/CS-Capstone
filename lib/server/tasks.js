...
async function getTaskCategories(taskId){
    ...
    try{
        if(await isRemoteAsync()){
            result = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_TASK_CATEGORIES, {taskId: taskId, username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});

        }
        else{
            result = await taskHandler.current.getTaskCategories(serverHandler.current.userState.username, taskId);
        }
    }
    catch (e) {
