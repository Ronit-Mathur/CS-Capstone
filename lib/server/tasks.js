...
async function leastEnjoyableTask(){
    ...
    try{
        if(await isRemoteAsync()){
            ...
            task = await taskHandler.current.leastEnjoyableRepetetiveTask(serverHandler.current.userState.username);
        }
    }
    catch (e) {
...
async function nextTask(){
    ...
    try{
        ...
        console.log("nextTask unable to get task from the server");
        console.log(e);
    }
    return tasks;
}
...
/**
 * adds a task to any category given
 * @param {*} taskId 
 * @param {*} category can be any string. will be created if dosent exist
 */
async function addTaskToCategory(taskId, category){
    var result = false;
    try{
        if(await isRemoteAsync()){
            result = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_TASK_ADD_TO_CATEGORY, {taskId: taskId, category:category,  username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});

        }
        else{
            result = await taskHandler.current.addTaskToCategory(serverHandler.current.userState.username, taskId, category);
        }
    }
    catch (e) {
        console.log("addTaskToCategory unable to get task from the server");
        console.log(e);
    }
    return result;
}

async function getTaskCategories(taskId){
    var cats = [];
    try{
        if(await isRemoteAsync()){
            cats = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_TASK_CATEGORIES, {taskId: taskId, username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});

        }
        else{
            cats = await taskHandler.current.getTaskCategories(serverHandler.current.userState.username, taskId);
        }
    }
    catch (e) {
        console.log("getTaskCategories unable to get task from the server");
        console.log(e);
    }
    return cats;
}
 ...
 * 
 * @param {*} taskId 
 * @returns all categories for which a user has tasks belonging too
 */
async function getAllTaskCategories(taskId){
    var cats = [];
    try{
        if(await isRemoteAsync()){
            cats = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_ALL_TASK_CATEGORIES, {username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});

        }
        else{
            cats = await taskHandler.current.getAllTaskCategories(serverHandler.current.userState.username);
        }
    }
    catch (e) {
        console.log("getAllTaskCategories unable to get task from the server");
        console.log(e);
    }
    return cats;
}
 ...
 * 
 * @param {*} category 
 * @returns a list of task ids belonging to a category and this user
 */
async function getAllTasksBelongingToCategory(category){
    var ids = [];
    try{
        if(await isRemoteAsync()){
            ids = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_TASKS_ALL_IN_CATEGORY, {category: category, username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});

        }
        else{
            ids = await taskHandler.current.getAllTasksBelongingToCategory(serverHandler.current.userState.username, category);
        }
    }
    catch (e) {
        console.log("getAllTasksBelongingToCategory unable to get task from the server");
        console.log(e);
    }
    return ids;
}
