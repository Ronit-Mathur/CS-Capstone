...
async function happiestWhenDayStartsWith(){
    ...
    try{
        if(await isRemoteAsync()){
            task = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.happiestWhenDayStartsWith, {username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});
...
export {happiestWhenDayStartsWith, tasksWithDate, totalRatedTasks, totalCompletedTasks, getRatedTasksByEngagement, getUnratedCompletedTasks, addTask, getDaysTasks, getTodaysFinishedTasks, getTodaysActiveTasks, updateTask, deleteTask, rateTask, getTaskRating, getTasksByMonth, getTask, getTaskRatingsByMonth }