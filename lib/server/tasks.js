...
async function addTask(user, summary, day, location, startTime, endTime) {
    ...
    try {
        ...
        taskId = await serverHandler.current.fetchGet(serverConstants.SERVER_ENDPOINTS.USER_ADD_TASK, { username: user, summary: summary, day: day, startTime: startTime, endTime: endTime, location: location, auth: serverHandler.current.userState.auth });
...
async function getUnratedCompletedTasks(){
    var tasks = [];
    try{
        tasks = await serverHandler.current.fetchGet(SERVER_ENDPOINTS.USER_TASKS_NOT_RATED, {username: serverHandler.current.userState.username, auth: serverHandler.current.userState.auth});
    }catch(e){
...
export { getRatedTasksByEngagement, getUnratedCompletedTasks, addTask, getDaysTasks, getTodaysFinishedTasks, getTodaysActiveTasks, updateTask, deleteTask, rateTask, getTaskRating, getTasksByMonth, getTask, getTaskRatingsByMonth }