...
module.exports = class taskHandler {
    ...
    */
    async getMonthsRatedTasks(month, username) {
        ...
        var yearNum = month.substring(3, 7);
        var glob = monthNum + "/??/" + yearNum;
        var q = new Query(1, "SELECT DISTINCT taskId, date FROM ((SELECT taskId as ratingId FROM ratedTasks) INNER JOIN tasks ON ratedTasks.taskId = tasks.taskId) WHERE  username = ? AND date GLOB ?",  [username, glob]);
